...
namespace ConfigTransformationTool
{
    ...
    class ArgumentsLoader
    {
        public bool AreAllRequiredParametersSet
        {
            get
            {
                return !string.IsNullOrWhiteSpace(this.SourceFilePath) && !string.IsNullOrWhiteSpace(this.TransformFilePath) && !string.IsNullOrWhiteSpace(this.DestinationFilePath);
            }
        }
        ...
        public bool Load(string[] args)
        {
            ...
            {
                if (arg.IndexOf("s:", StringComparison.OrdinalIgnoreCase) == 0 || arg.IndexOf("source:", StringComparison.OrdinalIgnoreCase) == 0)
                {
                    this.SourceFilePath = GetValueFromArguments(arg);
                    continue;
                }

                if (arg.IndexOf("t:", StringComparison.OrdinalIgnoreCase) == 0 || arg.IndexOf("transform:", StringComparison.OrdinalIgnoreCase) == 0)
                {
                    this.TransformFilePath = GetValueFromArguments(arg);
                    continue;
                }

                if (arg.IndexOf("d:", StringComparison.OrdinalIgnoreCase) == 0 || arg.IndexOf("destination:", StringComparison.OrdinalIgnoreCase) == 0)
                {
                    this.DestinationFilePath = GetValueFromArguments(arg);
                    continue;
                }

                if (arg.IndexOf("p:", StringComparison.OrdinalIgnoreCase) == 0 || arg.IndexOf("parameters:", StringComparison.OrdinalIgnoreCase) == 0)
                {
                    this.ParametersString = GetValueFromArguments(arg);
                    continue;
                }

                if (arg.IndexOf("pf:", StringComparison.OrdinalIgnoreCase) == 0 || arg.IndexOf("parameters.file:", StringComparison.OrdinalIgnoreCase) == 0)
                {
                ...
                if (arg.Equals("i", StringComparison.OrdinalIgnoreCase) || arg.Equals("indent", StringComparison.OrdinalIgnoreCase))
                {
                    this.Indent = true;
                    continue;
                }

                if (arg.IndexOf("ic:", StringComparison.OrdinalIgnoreCase) == 0 || arg.IndexOf("indentchars:", StringComparison.OrdinalIgnoreCase) == 0)
                {
                    this.IndentChars = GetValueFromArguments(arg);
                    continue;
                }

                if (arg.IndexOf("e:", StringComparison.OrdinalIgnoreCase) == 0 || arg.IndexOf("encoding:", StringComparison.OrdinalIgnoreCase) == 0)
                {
        ...
        static string GetValueFromArguments(string arg)
        {
